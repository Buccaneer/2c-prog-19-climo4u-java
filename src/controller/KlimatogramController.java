...
public class KlimatogramController implements Subject
{
    ...
    public void voegKlimatogramToe(KlimatogramDto klimatogramDto)
    {
        ...
        for (int i = 0; i < 12; i++)
        {
            try { klim.getMaanden().get(i).setNeerslag(klimatogramDto.maanden.get(i).getNeerslag()); } 
            catch (IllegalArgumentException e) { vie.add("neerslagMaand" + (i + 1), e); }
            try { klim.getMaanden().get(i).setTemperatuur(klimatogramDto.maanden.get(i).getTemperatuur()); }
            catch (IllegalArgumentException e) { vie.add("temperatuurMaand" + (i + 1), e); }
        }
        if (!vie.isEmpty())
            throw vie;
    ...
    public void wijzigKlimatogram(KlimatogramDto klimatogramDto) throws IllegalArgumentException, VerkeerdeInputException
    {
        ...
        for (int i = 0; i < 12; i++)
        {
            try { geselecteerdKlimatogram.getMaanden().get(i).setNeerslag(klimatogramDto.maanden.get(i).getNeerslag()); } 
            catch (IllegalArgumentException e) { vie.add("neerslagMaand" + (i + 1), e); }
            try { geselecteerdKlimatogram.getMaanden().get(i).setTemperatuur(klimatogramDto.maanden.get(i).getTemperatuur()); }
            catch (IllegalArgumentException e) { vie.add("temperatuurMaand" + (i + 1), e); }
        }
        if (!vie.isEmpty())
            throw vie;    
